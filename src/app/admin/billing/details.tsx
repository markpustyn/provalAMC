...
const BillingDetails: React.FC<BillingStatusProps> = ({ BillingDetails }) => {

  if (!BillingDetails) return <div className="text-center text-gray-500 text-sm">Loading...</div>;

  return (
    <div className="bg-white shadow-md rounded-lg p-4 space-y-4">
      <h2 className="text-lg font-semibold text-gray-800 border-b pb-1">Vendor Details</h2>

      {/* Personal Information */}
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <p className="text-gray-500">First Name</p>
          <p className="font-medium">{BillingDetails.propOrderId || "N/A"}</p>
        </div>
        <div>
          <p className="text-gray-500">Last Name</p>
          <p className="font-medium">{BillingDetails.amount || "N/A"}</p>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <p className="text-gray-500">Email</p>
          <p className="font-medium">{BillingDetails.vendorFee || "N/A"}</p>
        </div>
        </div>
      {/* <div>
