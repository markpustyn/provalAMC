...
CREATE TABLE "order" (
	...
	"description" text,
	"order_status" varchar DEFAULT 'open'
);
--> statement-breakpoint
...
CREATE TABLE "users" (
	...
	"role" "roles" DEFAULT 'broker',
	"statued" "status" DEFAULT 'active',
...
ALTER TABLE "order" ADD CONSTRAINT "order_order_status_statusOrder_prop_status_fk" FOREIGN KEY ("order_status") REFERENCES "public"."statusOrder"("prop_status") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
